...
class TaskRepository {
    ...
    async findById(id) {
        const { rows } = await this.db.query(`SELECT id, title, description, status, project_id, assigned_to, due_date, created_at
    ...
    async update(id, data) {
       ...
       RETURNING id, title, description, status, project_id, assigned_to, due_date, created_at`, values);
        return rows[0];
    }
    async delete(id) {
