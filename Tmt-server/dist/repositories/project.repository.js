...
class ProjectRepository {
    ...
    async findById(id) {
        const { rows } = await this.db.query(`SELECT id, name, description, created_by, created_at FROM projects WHERE id = $1`, [id]);
        return rows[0] ?? null;
    }
    ...
    async create(data) {
        ...
        try {
         ...
         VALUES ($1, $2, $3)
         RETURNING id, name, description, created_by, created_at`, [data.name, data.description ?? null, data.createdBy]);
    ...
    async update(id, data) {
        ...
        try {
           ...
           RETURNING id, name, description, created_by, created_at`, values);
                project = rows[0];
            }
            else {
                const { rows } = await client.query(`SELECT id, name, description, created_by, created_at FROM projects WHERE id = $1`, [id]);
                project = rows[0];
            }
