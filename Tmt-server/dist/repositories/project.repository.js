...
class ProjectRepository {
    ...
    async findById(id) {
        const { rows } = await this.db.query(`SELECT id, name, description, created_by AS "createdBy", created_at AS "createdAt" FROM projects WHERE id = $1`, [id]);
        return rows[0] ?? null;
    }
    ...
    async create(data) {
        ...
        try {
            ...
            const { rows } = await client.query(`INSERT INTO projects (name, description, created_by)
         VALUES ($1, $2, $3)
         RETURNING id, name, description, created_by AS "createdBy", created_at AS "createdAt"`, [data.name, data.description ?? null, data.createdBy]);
            const project = rows[0];
            const memberIds = Array.from(new Set([data.createdBy, ...(data.memberIds ?? [])]));
            if (memberIds.length) {
                await client.query(`INSERT INTO project_members (project_id, user_id)
           SELECT $1, unnest($2::uuid[])
           ON CONFLICT DO NOTHING`, [project.id, memberIds]);
            }
    ...
    async update(id, data) {
        ...
        try {
            ...
            if (fields.length > 0) {
                values.push(id);
                const { rows } = await client.query(`UPDATE projects SET ${fields.join(', ')} WHERE id = $${idx}
           RETURNING id, name, description, created_by AS "createdBy", created_at AS "createdAt"`, values);
                project = rows[0];
            }
            else {
                const { rows } = await client.query(`SELECT id, name, description, created_by AS "createdBy", created_at AS "createdAt" FROM projects WHERE id = $1`, [id]);
                project = rows[0];
            }
            ...
            if (data.memberIds !== undefined) {
                ...
                const { rows: creatorRows } = await client.query(`SELECT created_by FROM projects WHERE id = $1`, [id]);
                const creatorId = creatorRows[0]?.created_by;
                const memberIds = Array.from(new Set([...(creatorId ? [creatorId] : []), ...data.memberIds]));
                if (memberIds.length > 0) {
                    await client.query(`INSERT INTO project_members (project_id, user_id)
             SELECT $1, unnest($2::uuid[])
             ON CONFLICT DO NOTHING`, [id, memberIds]);
                }
            }
            await client.query('COMMIT');
            return project;
