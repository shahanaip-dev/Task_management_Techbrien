...
class UserRepository {
    ...
    async findById(id) {
        const { rows } = await this.db.query(`SELECT id, name, email, password, role, created_at FROM users WHERE id = $1`, [id]);
        return rows[0] ?? null;
    }
    ...
    async findByEmail(email) {
        const { rows } = await this.db.query(`SELECT id, name, email, password, role, created_at FROM users WHERE email = $1`, [email.toLowerCase().trim()]);
        return rows[0] ?? null;
    }
